...
cron.schedule("* * * * *", async () => {
  try {
    ...
    // Number(process.env.EXPIRE_VENUE_APPROVAL_TIMING) || 1;
    const expireVenuePaymentTiming = 30;
    // Number(process.env.EXPIRE_VENUE_PAYMENT_TIMING) || 1;
    const expireVenueRequestTiming =
      // Number(process.env.EXPIRE_VENUE_REQUEST_TIMING) ||
    ...
    const { data: expiredVenueRequests, error: venueRequestError } =
      await supabase
        .from("venue_requests")
        .update({
          status: "expired",
