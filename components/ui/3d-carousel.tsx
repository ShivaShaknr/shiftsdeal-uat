...
const Carousel = memo(
  ({
    ...
    // Slow continuous auto-rotation; pauses while the user is dragging.
    useAnimationFrame((_, delta) => {
      if (!isCarouselActive || isDraggingRef.current) return;
      rotation.set(rotation.get() + (delta / 1000) * 6);
    });
      ...
      <AnimatePresence mode="sync">
        {activeImg && (
          <motion.div
            ...
            layout="position"
            onClick={handleClose}
            className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 m-5 md:m-36 lg:mx-[19rem] rounded-3xl"
