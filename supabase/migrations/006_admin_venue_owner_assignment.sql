-- Admin-side venue onboarding + owner assignment foundation
-- Adds scalable ownership mapping and admin-only venue metadata columns.
  ...
  IF NOT EXISTS (
    ...
    WITH CHECK (
      ...
      AND (
        ...
        OR
        -- Owners can upload for their own venues (path: venue-{venue_id}/* or similar)
        EXISTS (
          SELECT 1 FROM public.venue_ownerships
